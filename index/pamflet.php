...
<div class="container floated">
				...
				<div class="color-1">
					...
					if(isset($_POST['search_publication'])){
					 ...
					 $excute = $mysqli->query($query);
					  while($data = mysqli_fetch_array($excute)){
					?>
	...
	if(abs((int)$_GET['publication_id']) == ""){
		$q_max = $mysqli->query("select max(publication_id) as new_id from publications where publication_type = '2' and active_status = '1'");
		$r_max = mysqli_fetch_array($q_max);
		$publication_id = $r_max['new_id'];
	}else{
    	$publication_id = abs((int)$_GET['publication_id']);
	}
	$query_judul = $mysqli->query("SELECT publication_title FROM publications where publication_id = '$publication_id'");
    $data_judul = mysqli_fetch_array($query_judul);
	?>
	<!-- Page Content -->
	<div class="eleven floated right">
		<div class="page-content">
			...
			<h3 class="margin-reset"><?php echo $data_judul['publication_title'] ?></h3><br>
			 ...
			 <?php
	$query_pub = $mysqli->query("SELECT * FROM publication_pic where publication_id = '$publication_id' order by pic_id asc");
    while($data_pub = mysqli_fetch_array($query_pub)){
	?>
			<p>
