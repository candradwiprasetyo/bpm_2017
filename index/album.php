...
<div class="container floated">
	...
	if(abs((int)$_GET['album_id']) == ""){
		$q_max = $mysqli->query("select max(album_id) as new_id from album");
		$r_max = mysqli_fetch_array($q_max);
		$album_id = $r_max['new_id'];
	}else{
    	$album_id = abs((int)$_GET['album_id']);
	}
	$query_judul = $mysqli->query("SELECT album_title FROM album where album_id = '$album_id'");
	...
	<div class="eleven floated right">
		<div class="page-content">
			...
			<h3 class="margin-reset"><?php echo $data_judul['album_title'] ?></h3><br>
			  ...
			  <?php
			$query_pub = $mysqli->query("SELECT * FROM album_pic where album_id = '$album_id'");
