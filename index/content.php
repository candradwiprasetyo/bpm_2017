<?php
global $mysqli;
$q_cat = $mysqli->query("select * from menus where id_menu ='".abs((int)$_GET['id_menu'])."'");
$r_cat = mysqli_fetch_array($q_cat);
...
<div class="container floated">
	...
	<div class="eleven floated">
		...
		if(!isset($_GET['news_id'])){
			$q_max = $mysqli->query("select max(news_id) as new_id from news_menu where news_cat_id = '".abs((int)$_GET['id_menu'])."' and active_status = '1'");
		...
		$query_pub = $mysqli->query("SELECT * FROM news_menu where news_id = '$news_id'");
	    $data_pub = mysqli_fetch_array($query_pub);
		?>
